 ...
 */
static bool all_currencies_available(sqlite3 * db,
                                     char *reporting_currency,
                                     char **missing_currencies);
...
int accounts_balance(PARAMETERS parameters)
{
    ...
    sprintf(sql_accounts_balance, "SELECT" " A.TYPE"    // 0
            ", A.NAME"          // 1
            ", sum(T.VALUE)"    // 2
            ", A.CURRENCY"      // 3
            " FROM"
            ...
            ";", year, year, month, year, month, day, ITEM_STAT_OPEN);
    if (sqlite3_prepare_v2
        (db, sql_accounts_balance, REP_SQL_SIZE, &sqlStmt,
         NULL) != SQLITE_OK) {
        fprintf(stderr, "%s: %s\n", parameters.prog_name,
                sqlite3_errmsg(db));
        return 1;
...
int assets_summary(PARAMETERS parameters)
{
    ...
    sprintf(sql_accounts_balance, "SELECT" " A.TYPE"    // 0
            ...
            " GROUP BY A.TYPE, A.NAME"
            " ORDER BY A.TYPE, A.NAME"
            ";", parameters.currency, year, year, month, year, month, day,
            parameters.currency, ITEM_STAT_OPEN);
    ...
    if (all_currencies_available
        (db, parameters.currency, &list_of_missing_currencies) == true) {
        if (sqlite3_prepare_v2
            (db, sql_accounts_balance, REP_SQL_SIZE, &sqlStmt,
             NULL) != SQLITE_OK) {
            fprintf(stderr, "%s: %s\n", parameters.prog_name,
        ...
        while ((rc = sqlite3_step(sqlStmt)) == SQLITE_ROW) {
            if (current_type != sqlite3_column_int(sqlStmt, 0)) {
                if (current_type != ACC_TYPE_UNSET) {
                    printf(EMP_ON FS_ATYPE "  " FS_GAP FS_VALUE FS_GAPS
                           FS_CUR EMP_OFF "\n",
                           account_type_text(current_type), subtotal,
                           parameters.currency);
                }
                current_type = sqlite3_column_int(sqlStmt, 0);
                printf("\n" FS_ATYPE "\n",
            ...
            printf(FS_GAP FS_NAME FS_GAP FS_VALUE FS_GAPS FS_CUR "\n",
                   sqlite3_column_text(sqlStmt, 1),
                   sqlite3_column_double(sqlStmt, 2), parameters.currency);
        ...
        printf("\n" EMP_ON FS_NAME "  " FS_GAP FS_VALUE FS_GAPS FS_CUR
               EMP_OFF "\n", "Total:", total, parameters.currency);
        ...
        fprintf(stderr, MSG_MISSING_EXCHANGE_RATES, parameters.prog_name,
                list_of_missing_currencies);
...
int budget_report_categories(PARAMETERS parameters)
{
    ...
    float value_budget = 0.0,
        value_actual = 0.0,
        subtotal_budget = 0.0,
        subtotal_actual = 0.0, total_budget = 0.0, total_actual = 0.0;
    char *list_of_missing_currencies = NULL;
    ...
    if (parameters.date[0] != NULL_STRING) {
        ...
        date_type =
            date_from_string(parameters.date_default, &year, &month, NULL);
    ...
    if (date_type == DT_MONTH) {
        ...
        sprintf(sql_categories_list, "(SELECT DISTINCT CATEGORY_ID"
                ...
                " FROM TRANSACTIONS"
                " WHERE YEAR=%d AND MONTH=%d)", year, month, year, month);
        sprintf(sql_budget, "(SELECT BUDGETS.CATEGORY_ID,"
                ...
                parameters.currency, parameters.currency, year, month);
        sprintf(sql_transactions,
                "(SELECT CATEGORY_ID"
                ...
                " WHERE YEAR=%d AND MONTH=%d" " GROUP BY CATEGORY_ID" ")",
                parameters.currency, parameters.currency, year, month);
        sprintf(sql_final,
                "SELECT" " MAIN_CATEGORIES.TYPE" ", MAIN_CATEGORIES.NAME"
                ", CATEGORIES.NAME"
                ...
                ";", sql_categories_list, sql_budget, sql_transactions);
        ...
        sprintf(sql_categories_list, "(SELECT DISTINCT CATEGORY_ID"
                ...
                " UNION "
                " SELECT DISTINCT CATEGORY_ID"
                " FROM TRANSACTIONS" " WHERE YEAR=%d)", year, year);
        sprintf(sql_budget, "(SELECT BUDGETS.CATEGORY_ID,"
                ...
                " GROUP BY BUDGETS.CATEGORY_ID)", parameters.currency,
                parameters.currency, year);
        sprintf(sql_transactions,
                "(SELECT CATEGORY_ID"
                ", sum(VALUE * (CASE ACCOUNTS.CURRENCY WHEN '%s' THEN 1.0 ELSE TRANCURRENCIES.EXCHANGE_RATE END)) as SPENT_VALUE"
                " FROM TRANSACTIONS"
                ...
                " LEFT JOIN (SELECT * FROM CURRENCIES WHERE CURRENCY_TO='%s') AS TRANCURRENCIES"
                " ON ACCOUNTS.CURRENCY=TRANCURRENCIES.CURRENCY_FROM"
                " WHERE YEAR=%d" " GROUP BY CATEGORY_ID" ")",
                parameters.currency, parameters.currency, year);
        sprintf(sql_final,
                "SELECT" " MAIN_CATEGORIES.TYPE" ", MAIN_CATEGORIES.NAME"
                ", CATEGORIES.NAME"
                ", CASE WHEN BUDGETLIMITS.BUDGET_LIMIT IS NULL THEN 0.0 ELSE BUDGETLIMITS.BUDGET_LIMIT END AS BUDGET"
    ...
    if (all_currencies_available
        ...
        if (sqlite3_prepare_v2(db, sql_final, REP_SQL_SIZE, &sqlStmt, NULL)
            != SQLITE_OK) {
            fprintf(stderr, "%s: %s\n", parameters.prog_name,
                    sqlite3_errmsg(db));
            return 1;
        ...
        MAINCATEGORY_TYPE prev_maincategory_type =
            CAT_TYPE_NOTSET, cur_maincategory_type;
        int subtotal_flag = 0;
        printf("%s\n", title);
        while ((rc = sqlite3_step(sqlStmt)) == SQLITE_ROW) {
            cur_maincategory_type = sqlite3_column_int(sqlStmt, 0);
            if (cur_maincategory_type == CAT_INCOME
                || cur_maincategory_type == CAT_COST) {
                if (prev_maincategory_type != cur_maincategory_type) {
                    if (subtotal_flag == 0) {
                        subtotal_flag = 1;
                    } else {
                        printf(EMP_ON FS_MTYPE "    " FS_GAP FS_NAME FS_GAP
                               FS_VALUE FS_GAPS FS_CUR FS_GAP FS_VALUE
                               FS_GAPS FS_CUR FS_GAP FS_VALUE FS_GAPS
                               FS_CUR EMP_OFF "\n",
                               maincategory_type_text
                               (prev_maincategory_type), "",
                               subtotal_budget, parameters.currency,
                               subtotal_actual, parameters.currency,
                    ...
                    printf(FS_GAP FS_NAME_T FS_GAP FS_NAME_T FS_GAP
                           FS_VALUE_T FS_GAPS FS_CUR_T FS_GAP FS_VALUE_T
                           FS_GAPS FS_CUR_T FS_GAP FS_VALUE_T FS_GAPS
                           FS_CUR_T "\n", "MAIN CAT.", "CATEGORY", "LIMIT",
                           "CUR", "ACTUAL", "CUR", "DIFFERENCE", "CUR");
                }
        ...
        total_budget += subtotal_budget;
        total_actual += subtotal_actual;
        printf(EMP_ON FS_MTYPE "    " FS_GAP FS_NAME FS_GAP FS_VALUE
               FS_GAPS FS_CUR FS_GAP FS_VALUE FS_GAPS FS_CUR FS_GAP
               FS_VALUE FS_GAPS FS_CUR EMP_OFF "\n",
               maincategory_type_text(prev_maincategory_type), "",
               subtotal_budget, parameters.currency, subtotal_actual,
               parameters.currency, subtotal_actual - subtotal_budget,
               parameters.currency);
        printf("\n" EMP_ON FS_MTYPE "    " FS_GAP FS_NAME FS_GAP FS_VALUE
               FS_GAPS FS_CUR FS_GAP FS_VALUE FS_GAPS FS_CUR FS_GAP
               FS_VALUE FS_GAPS FS_CUR EMP_OFF "\n", "Total", "",
               total_budget, parameters.currency, total_actual,
               parameters.currency, total_actual - total_budget,
               parameters.currency);
...
int budget_report_maincategories(PARAMETERS parameters)
{
    ...
    if (date_type == DT_MONTH) {
        sprintf(title, "Budget report for %d-%02d:", year, month);
        sprintf(sql_maincategories_list,
                "(SELECT DISTINCT MCL_C.MAIN_CATEGORY_ID AS MCL_MAIN_CATEGORY_ID FROM CATEGORIES AS MCL_C"
                " INNER JOIN (SELECT DISTINCT CATEGORY_ID" " FROM BUDGETS"
                " WHERE YEAR=%d AND MONTH=%d" " UNION "
        ...
        sprintf(sql_budget,
                "(SELECT B_C.MAIN_CATEGORY_ID, sum(B_BC.B_BUDGET_LIMIT)"
                ...
                " GROUP BY B_C.MAIN_CATEGORY_ID)", parameters.currency,
                parameters.currency, year, month);
        sprintf(sql_transactions,
                "(SELECT T_C.MAIN_CATEGORY_ID"
                ...
                " WHERE YEAR=%d AND MONTH=%d"
                " GROUP BY T_C.MAIN_CATEGORY_ID" ")", parameters.currency,
                parameters.currency, year, month);
        sprintf(sql_final,
                "SELECT" " MAIN_CATEGORIES.TYPE" ", MAIN_CATEGORIES.NAME"
                ...
                " ORDER BY MAIN_CATEGORIES.TYPE DESC, MAIN_CATEGORIES.NAME ASC"
                ";", sql_maincategories_list, sql_budget,
                sql_transactions);
        ...
        sprintf(sql_maincategories_list,
                "(SELECT DISTINCT MCL_C.MAIN_CATEGORY_ID AS MCL_MAIN_CATEGORY_ID FROM CATEGORIES AS MCL_C"
                " INNER JOIN (SELECT DISTINCT CATEGORY_ID" " FROM BUDGETS"
                " WHERE YEAR=%d" " UNION " " SELECT DISTINCT CATEGORY_ID"
                " FROM TRANSACTIONS"
        ...
        sprintf(sql_transactions,
                ...
                " ON T_A.CURRENCY=TRANCURRENCIES.CURRENCY_FROM"
                " INNER JOIN CATEGORIES AS T_C ON T_C.CATEGORY_ID=T_T.CATEGORY_ID"
                " WHERE YEAR=%d" " GROUP BY T_C.MAIN_CATEGORY_ID" ")",
                parameters.currency, parameters.currency, year);
        sprintf(sql_final,
                "SELECT" " MAIN_CATEGORIES.TYPE" ", MAIN_CATEGORIES.NAME"
                ", CASE WHEN BUDGETLIMITS.B_BUDGET_MAINCATS_LIMIT IS NULL THEN 0.0 ELSE BUDGETLIMITS.B_BUDGET_MAINCATS_LIMIT END AS BUDGET"
    ...
    if (all_currencies_available
            ...
            if (cur_maincategory_type == CAT_INCOME
                ...
                if (prev_maincategory_type != cur_maincategory_type) {
                    if (subtotal_flag == 0) {
                        ...
                        printf(EMP_ON FS_MTYPE "    " FS_GAP FS_VALUE
                               FS_GAPS FS_CUR FS_GAP FS_VALUE FS_GAPS
                               FS_CUR FS_GAP FS_VALUE FS_GAPS FS_CUR
                               EMP_OFF "\n",
                               maincategory_type_text
                               (prev_maincategory_type), subtotal_budget,
                               parameters.currency, subtotal_actual,
                               parameters.currency,
                               subtotal_actual - subtotal_budget,
                               parameters.currency);
                    }
                    ...
                    printf("\n" FS_MTYPE "\n",
                           maincategory_type_text(cur_maincategory_type));
                    printf(FS_GAP FS_NAME_T FS_GAP FS_VALUE_T FS_GAPS
                           FS_CUR_T FS_GAP FS_VALUE_T FS_GAPS FS_CUR_T
                           FS_GAP FS_VALUE_T FS_GAPS FS_CUR_T "\n",
                           "MAIN CAT.", "LIMIT", "CUR", "ACTUAL", "CUR",
                           "DIFFERENCE", "CUR");
                }
                printf(FS_GAP FS_NAME, sqlite3_column_text(sqlStmt, 1));
        ...
        total_budget += subtotal_budget;
        total_actual += subtotal_actual;
        printf(EMP_ON FS_MTYPE "    " FS_GAP FS_VALUE FS_GAPS FS_CUR FS_GAP
               FS_VALUE FS_GAPS FS_CUR FS_GAP FS_VALUE FS_GAPS FS_CUR
               EMP_OFF "\n",
               maincategory_type_text(prev_maincategory_type),
               subtotal_budget, parameters.currency, subtotal_actual,
               parameters.currency, subtotal_actual - subtotal_budget,
               parameters.currency);
        printf("\n" EMP_ON FS_MTYPE "    " FS_GAP FS_VALUE FS_GAPS FS_CUR
               FS_GAP FS_VALUE FS_GAPS FS_CUR FS_GAP FS_VALUE FS_GAPS
               FS_CUR EMP_OFF "\n", "Total", total_budget,
               parameters.currency, total_actual, parameters.currency,
               total_actual - total_budget, parameters.currency);
...
int categories_balance(PARAMETERS parameters)
{
    ...
    if (parameters.date[0] != NULL_STRING) {
        if ((date_type =
             date_from_string(parameters.date, &year, &month,
                              &day)) == DT_NO_DATE) {
            fprintf(stderr, MSG_WRONG_DATE_FULL, parameters.prog_name);
            return 1;
        } else {
            ...
            case DT_FULL_DATE:
                ...
                sprintf(balance_title,
                        "Category summary up to: %d-%02d-%02d", year,
                        month, day);
                break;
            case DT_YEAR:
                sprintf(sql_transactions_subquery,
                        "(SELECT * FROM TRANSACTIONS WHERE YEAR=%d)",
                        year);
                sprintf(balance_title, "Category summary during year %d",
                        year);
                break;
            case DT_MONTH:
                sprintf(sql_transactions_subquery,
                        "(SELECT * FROM TRANSACTIONS WHERE YEAR=%d AND MONTH=%d)",
                        year, month);
                sprintf(balance_title, "Category during month %d-%02d",
                        year, month);
                break;
            case DT_NO_DATE:
                break;
        ...
        if ((date_type =
             date_from_string(parameters.date_default, &year, &month,
                              &day)) == DT_NO_DATE) {
            fprintf(stderr, MSG_WRONG_DATE_FULL, parameters.prog_name);
            return 1;
        } else {
            sprintf(sql_transactions_subquery,
                    ...
                    " SELECT * FROM TRANSACTIONS WHERE YEAR=%d AND MONTH=%d AND DAY<=%d)",
                    year, year, month, year, month, day);
            sprintf(balance_title, "Category summary up to: %d-%02d-%02d",
                    year, month, day);
        }
    }

    // Prepare subquery for currencies
    sprintf(sql_currencies_subquery,
            "(SELECT * FROM CURRENCIES WHERE CURRENCY_TO='%s')",
            parameters.currency);
    ...
    sprintf(sql_categories_balance,
            ...
            parameters.currency, sql_transactions_subquery,
    ...
    if (all_currencies_available
        (db, parameters.currency, &list_of_missing_currencies) == true) {
        if (sqlite3_prepare_v2
            (db, sql_categories_balance, REP_SQL_SIZE, &sqlStmt,
             NULL) != SQLITE_OK) {
            fprintf(stderr, "%s: %s\n", parameters.prog_name,
                    sqlite3_errmsg(db));
            return 1;
            ...
            if (maincategory_type != sqlite3_column_int(sqlStmt, 0)) {
                if (subtotal_flag == 0) {
                    ...
                    printf(EMP_ON FS_MTYPE "    " FS_GAP FS_NAME FS_GAP
                           ...
                           maincategory_balance, parameters.currency);
                }
                maincategory_balance = 0.0;
                maincategory_type = sqlite3_column_int(sqlStmt, 0);
                printf("\n" FS_MTYPE "\n",
                       maincategory_type_text(maincategory_type));
                printf(FS_GAP FS_NAME_T FS_GAP FS_NAME_T FS_GAP FS_VALUE_T
                       FS_GAPS FS_CUR_T "\n", "MAIN CAT.", "CATEGORY",
                       "VALUE", "CUR");
            }
            printf(FS_GAP FS_NAME, sqlite3_column_text(sqlStmt, 1));
...
int maincategories_balance(PARAMETERS parameters)
{
    ...
    // Prepare final query
    sprintf(sql_categories_balance, "SELECT" " mc.TYPE" // 0
            ", mc.NAME"         // 1
            ", sum(CASE a.CURRENCY WHEN '%s' then 1 ELSE r.EXCHANGE_RATE END * t.VALUE) as CAT_VALUE"   // 2
            " FROM %s t"
            " INNER JOIN ACCOUNTS a ON t.ACCOUNT_ID=a.ACCOUNT_ID"
            ...
            " ORDER BY mc.TYPE DESC, mc.NAME ASC;", parameters.currency,
            sql_transactions_subquery, sql_currencies_subquery);
    ...
    if (all_currencies_available
        ...
        while ((rc = sqlite3_step(sqlStmt)) == SQLITE_ROW) {
            if (maincategory_type != sqlite3_column_int(sqlStmt, 0)) {
                if (subtotal_flag == 0) {
                    ...
                    printf(EMP_ON FS_MTYPE "    " FS_GAP FS_VALUE FS_GAPS
                           FS_CUR EMP_OFF "\n",
                           maincategory_type_text(maincategory_type),
                           maincategory_balance, parameters.currency);
                }
        ...
        printf(EMP_ON FS_MTYPE "    " FS_GAP FS_VALUE FS_GAPS FS_CUR
               EMP_OFF "\n", maincategory_type_text(maincategory_type),
               maincategory_balance, parameters.currency);
        printf("\n" EMP_ON FS_MTYPE "    " FS_GAP FS_VALUE FS_GAPS FS_CUR
               EMP_OFF "\n", "Total", total_balance, parameters.currency);
...
int transactions_balance(PARAMETERS parameters)
{
    ...
    if (all_currencies_available
        ...
        if (parameters.date[0] != NULL_STRING) {
            if ((date_type =
                ...
                case DT_FULL_DATE:
                    sprintf(sql_transactions_subquery,
                            ...
                            year, year, month, year, month, day);
                    sprintf(balance_title,
                            "Transactions up to: %d-%02d-%02d", year,
                            month, day);
                    break;
                case DT_YEAR:
                    sprintf(sql_transactions_subquery,
                            "(SELECT * FROM TRANSACTIONS WHERE YEAR=%d)",
                            year);
                    sprintf(balance_title, "Transactions during year %d",
                            year);
                    break;
                case DT_MONTH:
                    sprintf(sql_transactions_subquery,
                            ...
                            year, month);
                    sprintf(balance_title,
                            "Transactions during month %d-%02d", year,
                            month);
                    break;
                case DT_NO_DATE:
                    break;
            ...
            if ((date_type =
                ...
                sprintf(sql_transactions_subquery,
                        ...
                        " SELECT * FROM TRANSACTIONS WHERE YEAR=%d AND MONTH=%d AND DAY<=%d)",
                        year, year, month, year, month, day);
                sprintf(balance_title, "Transactions up to: %d-%02d-%02d",
                        year, month, day);
            }
        }
        ...
        // Prepare final query
        sprintf(sql_transactions_balance, "SELECT" " u.NAME"    // 0
                ", c.NAME"      // 1
                ", a.NAME"      // 2
                ", a.CURRENCY"  // 3
                ", t.YEAR"      // 4
                ...
                ";", parameters.currency, sql_transactions_subquery,
                sql_account_subquery, sql_currencies_subquery,
                sql_category_subquery, sql_maincategory_subquery);



        if (sqlite3_prepare_v2
            (db, sql_transactions_balance, REP_SQL_SIZE, &sqlStmt,
             NULL) != SQLITE_OK) {
            fprintf(stderr, "%s: %s\n", parameters.prog_name,
                    sqlite3_errmsg(db));
            return 1;
        ...
        // Print transactions on standard output
        printf("%s\n", balance_title);
        printf("\n" FS_DATE_T FS_GAP FS_NAME_T FS_GAP FS_NAME_T FS_GAP
               FS_NAME_T FS_GAP FS_VALUE_T FS_GAPS FS_CUR_T FS_GAP
               FS_DESC_T "\n", "DATE", "MAIN CAT.", "CATEGORY", "ACCOUNT",
...
int net_value(PARAMETERS parameters)
{
    ...
    sprintf(sql_net_value, "SELECT"
            ...
            " GROUP BY YEAR, MONTH"
            " ORDER BY YEAR, MONTH;", parameters.currency,
            parameters.currency);
    ...
    if (all_currencies_available
        (db, parameters.currency, &list_of_missing_currencies) == true) {
        if (sqlite3_prepare_v2
            (db, sql_net_value, REP_SQL_SIZE, &sqlStmt, NULL)
            != SQLITE_OK) {
            fprintf(stderr, "%s: %s\n", parameters.prog_name,
                    sqlite3_errmsg(db));
            return 1;
        ...
        while ((rc = sqlite3_step(sqlStmt)) == SQLITE_ROW) {
            ...
            printf(FS_MONTH FS_GAP FS_VALUE FS_GAPS FS_CUR "\n",
                   ...
                   sqlite3_column_int(sqlStmt, 1), net_value,
                   parameters.currency);
        }
        if (rc != SQLITE_DONE) {
            fprintf(stderr, "%s: %s\n", parameters.prog_name,
...
static bool
all_currencies_available(sqlite3 * db, char *reporting_currency,
                         char **missing_currencies)
{
    ...
    if (sqlite3_prepare_v2(db, sql_query, REP_SQL_SIZE, &sqlStmt_loc, NULL)
        == SQLITE_OK) {
        while ((rc = sqlite3_step(sqlStmt_loc)) == SQLITE_ROW) {
            result = false;
            if ((tmp_ptr =
                 realloc(*missing_currencies,
                         (++amount_of_items) * CUR_ITEM_LEN *
                         sizeof(char))) != NULL) {
                if (*missing_currencies == NULL) {
                    ...
                    *missing_currencies = tmp_ptr;
                    sprintf(currencies_item, ", %s-%s",
                            sqlite3_column_text(sqlStmt_loc, 0),
                            sqlite3_column_text(sqlStmt_loc, 1));
                    strncat(*missing_currencies, currencies_item,
                            CUR_ITEM_LEN);
                }
            }
        }
