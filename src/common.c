...
long account_id_for_name(sqlite3 * db, char *acc_name)
{
    ...
    // Prepare SQL query
    sprintf(sql_getId,
            "SELECT ACCOUNT_ID FROM ACCOUNTS WHERE NAME LIKE '%%%s%%' AND STATUS=%d;",
            acc_name, ITEM_STAT_OPEN);

    if (sqlite3_prepare_v2(db, sql_getId, COMMON_SQL_SIZE, &sqlStmt, NULL)
        != SQLITE_OK) {
        result = -1;
    }
    // Count accounts
...
int category_factor_for_id(sqlite3 * db, int cat_id)
{
    ...
    sprintf(sql_getType, "SELECT M.TYPE"
            ...
            " WHERE C.CATEGORY_ID=%d;", cat_id);
    if (sqlite3_prepare_v2
        (db, sql_getType, COMMON_SQL_SIZE, &sqlStmt, NULL) != SQLITE_OK) {
...
long category_id_for_name(sqlite3 * db, char *cat_name)
{
    ...
    // Prepare SQL query
    sprintf(sql_getId,
            "SELECT CATEGORY_ID FROM CATEGORIES WHERE NAME LIKE '%%%s%%' AND STATUS=%d;",
            cat_name, ITEM_STAT_OPEN);
...
DATE_TYPE
date_from_string(char *date_string, int *year_holder, int *month_holder,
                 int *day_holder)
{
...
long maincategory_id_for_name(sqlite3 * db, char *maincategory_name)
{
    ...
    // Prepare SQL query
    sprintf(sql_getId,
            "SELECT MAIN_CATEGORY_ID FROM MAIN_CATEGORIES WHERE NAME LIKE '%%%s%%' AND STATUS=%d;",
            maincategory_name, ITEM_STAT_OPEN);
    if (sqlite3_prepare_v2(db, sql_getId, COMMON_SQL_SIZE, &sqlStmt, NULL)
        != SQLITE_OK) {
        result = -1;
    }
...
unsigned long
transaction_category_id(sqlite3 * db, unsigned long transaction_id)
{
    ...
    // Prepare SQL query
    sprintf(sql_getCategoryID,
            "SELECT CATEGORY_ID FROM TRANSACTIONS WHERE TRANSACTION_ID=%li;",
            transaction_id);
    if (sqlite3_prepare_v2
        (db, sql_getCategoryID, COMMON_SQL_SIZE, &sqlStmt,
         NULL) != SQLITE_OK) {
        result = -1;
    }
    // Take type
