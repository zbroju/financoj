 ...
 * @return true or false
 */
static bool budget_exists(sqlite3 * db, const unsigned long category_id,
 ...
 * @return
 */
static bool currency_exists(sqlite3 * db, const char *currency_from,
 ...
 * @return true or false
 */
static bool maincategory_exists(sqlite3 * db,
                                const unsigned long maincategory_id);
 ...
 * @return true or false
 */
static bool transaction_exists(sqlite3 * db,
                               const unsigned long transaction_id);
...
int account_add(PARAMETERS parameters)
{
    ...
    if (parameters.dataFilePath[0] == NULL_STRING) {
        ...
        if (parameters.default_currency[0] != NULL_STRING) {
            strncpy(parameters.currency, parameters.default_currency,
                    PAR_CURRENCY_LEN - 1);
        } else {
            fprintf(stderr, MSG_MISSING_PAR_CURRENCY, parameters.prog_name,
                    OPTION_CURRENCY_SHORT, OPTION_CURRENCY_LONG);
            return 1;
        }
    }
    // Set default values
    account_type =
        (parameters.account_type ==
         ACC_TYPE_UNSET) ? ACC_TRANSACTIONAL : parameters.account_type;
    ...
    sprintf(sql_add_account, "INSERT INTO ACCOUNTS VALUES (" "null, "   // id - auto increment
            "'%s', "            // 1 - name
            "'%s', "            // 2 - description
            "'%s', "            // 3 - institution
            "%d, "              // 4 - account type
            ...
            parameters.institution, account_type, parameters.currency,
            ITEM_STAT_OPEN);

    if (sqlite3_exec(db, sql_add_account, NULL, NULL, &zErrMsg) !=
        SQLITE_OK) {
...
int account_edit(PARAMETERS parameters)
{
    ...
    if (parameters.dataFilePath[0] == NULL_STRING) {
        ...
        fprintf(stderr, MSG_MISSING_PAR_ID, parameters.prog_name,
                OPTION_ID_SHORT, OPTION_ID_LONG);
        return 1;
    } else if (parameters.currency[0] != NULL_STRING) {
        fprintf(stderr, MSG_CURRENCY_CANNOT_CHANGE, parameters.prog_name,
                OPTION_CURRENCY_SHORT, OPTION_CURRENCY_LONG);
        return 1;
    }
    // Open database file
    ...
    if (account_exists(db, parameters.id) == true) {
        ...
        if (parameters.name[0] != NULL_STRING) {
            sprintf(buf,
                    "UPDATE ACCOUNTS SET NAME='%s' WHERE ACCOUNT_ID=%li;",
        ...
        if (parameters.description[0] != NULL_STRING) {
            sprintf(buf,
                    "UPDATE ACCOUNTS SET DESCRIPTION='%s' WHERE ACCOUNT_ID=%li;",
                    parameters.description, parameters.id);
            strncat(sql_edit_account, buf, BUF_SIZE);
        }

        if (parameters.institution[0] != NULL_STRING) {
            sprintf(buf,
                    "UPDATE ACCOUNTS SET INSTITUTION='%s' WHERE ACCOUNT_ID=%li;",
                    parameters.institution, parameters.id);
            strncat(sql_edit_account, buf, BUF_SIZE);
        }

        if (parameters.account_type != ACC_TYPE_UNSET) {
            sprintf(buf,
                    "UPDATE ACCOUNTS SET TYPE=%d WHERE ACCOUNT_ID=%li;",
                    parameters.account_type, parameters.id);
            strncat(sql_edit_account, buf, BUF_SIZE);
        }

        strcat(sql_edit_account, "COMMIT;");

        if (sqlite3_exec(db, sql_edit_account, NULL, NULL, &zErrMsg) !=
            SQLITE_OK) {
            fprintf(stderr, "%s: %s\n", parameters.prog_name, zErrMsg);
            sqlite3_free(zErrMsg);
            result = 1;
            sprintf(final_message,
                    "%s: no change done for account with id=%li\n",
        ...
        result = 1;
        sprintf(final_message,
                "%s: there is no account with id=%li - no change done\n",
                parameters.prog_name, parameters.id);
    }
...
int account_list(PARAMETERS parameters)
{
    ...
    sprintf(sql_list_accounts, "SELECT"
            ...
            " FROM ACCOUNTS WHERE STATUS=%d", ITEM_STAT_OPEN);
    if (parameters.account_type != ACC_TYPE_UNSET) {
    ...
    if (sqlite3_prepare_v2(db, sql_list_accounts, SQL_SIZE, &sqlStmt, NULL)
        != SQLITE_OK) {
        fprintf(stderr, "%s: %s\n", parameters.prog_name,
                sqlite3_errmsg(db));
        return 1;
    ...
    printf(FS_ID_T FS_GAP FS_NAME_T FS_GAP FS_ATYPE_T FS_GAP FS_CUR_T
           FS_GAP FS_INST_T FS_GAP FS_DESC_T "\n", "ID", "NAME", "TYPE",
           "CUR", "BANK", "DESCRIPTION");
    ...
    while ((rc = sqlite3_step(sqlStmt)) == SQLITE_ROW) {
        ...
        printf(FS_GAP FS_ATYPE,
               account_type_text(sqlite3_column_int(sqlStmt, 2)));
...
int category_add(PARAMETERS parameters)
{
    ...
    maincategory_id =
        maincategory_id_for_name(db, parameters.maincat_name);
    if (maincategory_id == -1) {
        fprintf(stderr, MSG_MAINCATEGORY_NOT_FOUND, parameters.prog_name,
                parameters.maincat_name);
        sqlite3_close(db);
        return 1;
    }
    // Prepare SQL query and perform database actions
    sprintf(sql_add_category, "INSERT INTO CATEGORIES VALUES (" "null, "        // id - auto increment
            "%li, "             // 1 - main category
            "'%s', "            // 2 - name
            "%d);",             // 3 - status
            maincategory_id, parameters.name, ITEM_STAT_OPEN);

    if (sqlite3_exec(db, sql_add_category, NULL, NULL, &zErrMsg) !=
        SQLITE_OK) {
        fprintf(stderr, "%s: %s\n", parameters.prog_name, zErrMsg);
        sqlite3_free(zErrMsg);
        result = 1;
...
int category_edit(PARAMETERS parameters)
{
    ...
    if (category_exists(db, parameters.id) == true) {
        ...
        if (parameters.maincat_name[0] != NULL_STRING) {
            ...
            if (maincategory_id == -1) {
                ...
                sprintf(buf, "UPDATE CATEGORIES SET MAIN_CATEGORY_ID=%li"
                        ...
                        maincategory_id, parameters.id);
                strncat(sql_edit_category, buf, BUF_SIZE);
            }
        }
        if (parameters.name[0] != NULL_STRING) {
            sprintf(buf,
                    "UPDATE CATEGORIES SET NAME='%s' WHERE CATEGORY_ID=%li;",
                    parameters.name, parameters.id);
            strncat(sql_edit_category, buf, BUF_SIZE);
        }
        strcat(sql_edit_category, "COMMIT;");

        if (sqlite3_exec(db, sql_edit_category, NULL, NULL, &zErrMsg) !=
            SQLITE_OK) {
            fprintf(stderr, "%s: %s\n", parameters.prog_name, zErrMsg);
            sqlite3_free(zErrMsg);
            result = 1;
            sprintf(final_message,
                    "%s: no change done for category with id=%li\n",
                    parameters.prog_name, parameters.id);
        } else {
        ...
        result = 1;
        sprintf(final_message,
                "%s: there is no category with id=%li - no change done\n",
                parameters.prog_name, parameters.id);
    }
...
int category_remove(PARAMETERS parameters)
{
    ...
    sprintf(sql_remove_category, "UPDATE CATEGORIES SET STATUS=%d"
            " WHERE CATEGORY_ID=%li;", ITEM_STAT_CLOSED, parameters.id);

    if (sqlite3_exec(db, sql_remove_category, NULL, NULL, &zErrMsg) !=
        SQLITE_OK) {
        fprintf(stderr, "%s: %s\n", parameters.prog_name, zErrMsg);
        sqlite3_free(zErrMsg);
        result = 1;
...
int category_list(PARAMETERS parameters)
{
    ...
    // Prepare SQL query and perform database actions
    sprintf(sql_list_categories, "SELECT" " C.CATEGORY_ID"      // 0
            ",M.TYPE"           // 1
            ",M.NAME"           // 2
            ",C.NAME"           // 3
            " FROM CATEGORIES C"
    ...
    if (parameters.maincat_name[0] != NULL_STRING) {
        sprintf(sql_buf, " AND M.NAME LIKE '%%%s%%'",
                parameters.maincat_name);
    ...
    strncat(sql_list_categories, " ORDER BY M.TYPE, M.NAME, C.NAME;",
            BUF_SIZE);

    if (sqlite3_prepare_v2
        (db, sql_list_categories, SQL_SIZE, &sqlStmt, NULL)
        != SQLITE_OK) {
        fprintf(stderr, "%s: %s\n", parameters.prog_name,
                sqlite3_errmsg(db));
        return 1;
...
int currency_add(PARAMETERS parameters)
{
    ...
    // Prepare SQL query and perform database actions
    sprintf(sql_add_currency, "INSERT INTO CURRENCIES VALUES (" "'%s'"  // currency from
            ",'%s'"             // currency to
            ",round(%f,4));",   // echange rate
            parameters.currency,
            parameters.currency_to, atof(parameters.value));
    if (sqlite3_exec(db, sql_add_currency, NULL, NULL, &zErrMsg) !=
        SQLITE_OK) {
        fprintf(stderr, "%s: %s\n", parameters.prog_name, zErrMsg);
        sqlite3_free(zErrMsg);
        result = 1;
    ...
    if (parameters.verbose) {
        if (result == 0) {
            printf("%s: added new currency exchange rate %s-%s\n",
                   parameters.prog_name, parameters.currency,
...
int currency_edit(PARAMETERS parameters)
{
    ...
    if (currency_exists(db, parameters.currency, parameters.currency_to) ==
        true) {
        ...
        sprintf(sql_edit_currency,
                ...
                parameters.currency, parameters.currency_to);

        if (sqlite3_exec(db, sql_edit_currency, NULL, NULL, &zErrMsg) !=
            SQLITE_OK) {
            fprintf(stderr, "%s: %s\n", parameters.prog_name, zErrMsg);
            sqlite3_free(zErrMsg);
            result = 1;
            sprintf(final_message,
                    "%s: no change done for exchange rate %s-%s\n",
                    parameters.prog_name, parameters.currency,
                    parameters.currency_to);
        } else {
        ...
        result = 1;
        sprintf(final_message,
                "%s: there is no currncies pair: %s-%s - no change done\n",
                parameters.prog_name, parameters.currency,
                parameters.currency_to);
    }
    // Close database file
...
int currency_remove(PARAMETERS parameters)
{
    ...
    if (sqlite3_exec(db, sql_remove_currency, NULL, NULL, &zErrMsg) !=
        SQLITE_OK) {
        fprintf(stderr, "%s: %s\n", parameters.prog_name, zErrMsg);
        sqlite3_free(zErrMsg);
        result = 1;
...
int currency_list(PARAMETERS parameters)
{
    ...
    if (sqlite3_prepare_v2
        (db, sql_list_currencies, SQL_SIZE, &sqlStmt, NULL)
        != SQLITE_OK) {
        fprintf(stderr, "%s: %s\n", parameters.prog_name,
                sqlite3_errmsg(db));
        return 1;
...
int datafile_init(PARAMETERS parameters)
{
    ...
    if (sqlite3_exec(db, sqlTransaction, NULL, NULL, &zErrMsg) !=
        SQLITE_OK) {
        fprintf(stderr, "%s: %s\n", parameters.prog_name, zErrMsg);
        sqlite3_free(zErrMsg);
        result = 1;
...
int maincategory_add(PARAMETERS parameters)
{
    ...
    maincategory_type =
        (parameters.maincategory_type ==
         CAT_TYPE_NOTSET) ? CAT_COST : parameters.maincategory_type;
    ...
    // Prepare SQL query and perform database actions
    sprintf(sql_add_maincategory, "INSERT INTO MAIN_CATEGORIES VALUES (" "null, "       // id - auto increment
            "'%d', "            // 1 - type
            "'%s', "            // 2 - name
            "'%d');",           // 3 - status
            maincategory_type, parameters.name, ITEM_STAT_OPEN);

    if (sqlite3_exec(db, sql_add_maincategory, NULL, NULL, &zErrMsg) !=
        SQLITE_OK) {
        fprintf(stderr, "%s: %s\n", parameters.prog_name, zErrMsg);
        sqlite3_free(zErrMsg);
        result = 1;
...
int maincategory_edit(PARAMETERS parameters)
{
    ...
    if (maincategory_exists(db, parameters.id) == true) {
        ...
        if (parameters.maincategory_type != CAT_TYPE_NOTSET) {
            sprintf(buf,
                    "UPDATE MAIN_CATEGORIES SET TYPE=%d WHERE MAIN_CATEGORY_ID=%li;",
                    parameters.maincategory_type, parameters.id);
            strncat(sql_edit_maincategory, buf, BUF_SIZE);
        }
        if (parameters.name[0] != NULL_STRING) {
            sprintf(buf,
                    "UPDATE MAIN_CATEGORIES SET NAME='%s' WHERE MAIN_CATEGORY_ID=%li;",
                    parameters.name, parameters.id);
            strncat(sql_edit_maincategory, buf, BUF_SIZE);
        }
        ...
        if (sqlite3_exec(db, sql_edit_maincategory, NULL, NULL, &zErrMsg)
            != SQLITE_OK) {
            fprintf(stderr, "%s: %s\n", parameters.prog_name, zErrMsg);
            sqlite3_free(zErrMsg);
            result = 1;
            sprintf(final_message,
                    "%s: no change done for main category with id=%li\n",
                    parameters.prog_name, parameters.id);
        } else {
            sprintf(final_message,
                    "%s: edited main category with id=%li\n",
                    parameters.prog_name, parameters.id);
        }
    } else {
        result = 1;
        sprintf(final_message,
                "%s: there is no main category with id=%li - no change done\n",
                parameters.prog_name, parameters.id);
    }
...
int maincategory_remove(PARAMETERS parameters)
{
    ...
    sprintf(sql_remove_maincategory, "UPDATE MAIN_CATEGORIES"
            " SET STATUS=%d WHERE MAIN_CATEGORY_ID=%li;", ITEM_STAT_CLOSED,
            parameters.id);

    if (sqlite3_exec(db, sql_remove_maincategory, NULL, NULL, &zErrMsg) !=
        SQLITE_OK) {
        fprintf(stderr, "%s: %s\n", parameters.prog_name, zErrMsg);
        sqlite3_free(zErrMsg);
        result = 1;
...
int maincategory_list(PARAMETERS parameters)
{
    ...
    if (sqlite3_prepare_v2
        (db, sql_list_maincategories, SQL_SIZE, &sqlStmt,
         NULL) != SQLITE_OK) {
        fprintf(stderr, "%s: %s\n", parameters.prog_name,
                sqlite3_errmsg(db));
        return 1;
    ...
    while ((rc = sqlite3_step(sqlStmt)) == SQLITE_ROW) {
        printf(FS_ID, sqlite3_column_int(sqlStmt, 0));
        printf(FS_GAP FS_MTYPE,
               maincategory_type_text(sqlite3_column_int(sqlStmt, 1)));
...
int transaction_add(PARAMETERS parameters)
{
    ...
    if (parameters.dataFilePath[0] == NULL_STRING) {
        ...
        fprintf(stderr, MSG_MISSING_PAR_ACCOUNT_NAME, parameters.prog_name,
                OPTION_ACCOUNT_SHORT, OPTION_ACCOUNT_LONG);
        return 1;
    } else if (parameters.description[0] == NULL_STRING) {
        fprintf(stderr, MSG_MISSING_PAR_DESCRIPTION, parameters.prog_name,
                OPTION_DESCRIPTION_SHORT, OPTION_DESCRIPTION_LONG);
        return 1;
    } else if (parameters.value[0] == NULL_STRING) {
        fprintf(stderr, MSG_MISSING_PAR_VALUE, parameters.prog_name,
                OPTION_VALUE_SHORT, OPTION_VALUE_LONG);
        return 1;
    } else if (parameters.cat_name[0] == NULL_STRING) {
        fprintf(stderr, MSG_MISSING_PAR_CATEGORY, parameters.prog_name,
    ...
    if (account_id == -1) {
        fprintf(stderr, MSG_ACCOUNT_NOT_FOUND, parameters.prog_name,
                parameters.acc_name);
        sqlite3_close(db);
        return 1;
    }
    ...
    if (category_id == -1) {
        fprintf(stderr, MSG_CATEGORY_NOT_FOUND, parameters.prog_name,
                parameters.cat_name);
        sqlite3_close(db);
        return 1;
    }
    if (parameters.date[0] == NULL_STRING) {
        if (date_from_string(parameters.date_default, &year, &month, &day)
            != DT_FULL_DATE) {
            fprintf(stderr, MSG_WRONG_DATE_FULL, parameters.prog_name);
            return 1;
        }
    } else {
        if (date_from_string(parameters.date, &year, &month, &day) !=
            DT_FULL_DATE) {
            fprintf(stderr, MSG_WRONG_DATE_FULL, parameters.prog_name);
            return 1;
        }
    ...
    transaction_value =
        category_factor_for_id(db, category_id) * atof(parameters.value);

    // Prepare SQL query and perform database operations
    sprintf(sql_add_transaction, "INSERT INTO TRANSACTIONS VALUES (" "null, "   // id - auto increment
            "%d, "              // year
            "%d, "              // month
            "%d, "              // day
            "%li, "             // account id
            ...
            day,
            account_id,
            parameters.description, transaction_value, category_id);

    if (sqlite3_exec(db, sql_add_transaction, NULL, NULL, &zErrMsg) !=
        SQLITE_OK) {
        fprintf(stderr, "%s: %s\n", parameters.prog_name, zErrMsg);
        sqlite3_free(zErrMsg);
        result = 1;
    ...
    if (transaction_exists(db, parameters.id) == true) {
        ...
        if (parameters.acc_name[0] != NULL_STRING) {
            ...
            if (account_id == -1) {
                ...
                sprintf(buf,
                        "UPDATE TRANSACTIONS SET ACCOUNT_ID=%li WHERE TRANSACTION_ID=%li;",
        ...
        if (parameters.description[0] != NULL_STRING) {
            sprintf(buf,
                    "UPDATE TRANSACTIONS SET DESCRIPTION='%s' WHERE TRANSACTION_ID=%li;",
                    parameters.description, parameters.id);
            strncat(sql_edit_transaction, buf, BUF_SIZE);
        }
        ...
        if (parameters.cat_name[0] != NULL_STRING) {
            ...
            if (category_id == -1) {
                ...
                sprintf(buf,
                        "UPDATE TRANSACTIONS SET CATEGORY_ID=%li WHERE TRANSACTION_ID=%li;",
        ...
        if (parameters.value[0] != NULL_STRING) {
            transaction_value =
                category_factor_for_id(db,
                                       category_id) *
                atof(parameters.value);
            sprintf(buf,
                    "UPDATE TRANSACTIONS SET VALUE=round(%f,2) WHERE TRANSACTION_ID=%li;",
                    transaction_value, parameters.id);
            strncat(sql_edit_transaction, buf, BUF_SIZE);
        }
        strcat(sql_edit_transaction, "COMMIT;");

        if (sqlite3_exec(db, sql_edit_transaction, NULL, NULL, &zErrMsg) !=
            SQLITE_OK) {
            fprintf(stderr, "%s: %s\n", parameters.prog_name, zErrMsg);
            sqlite3_free(zErrMsg);
            result = 1;
            sprintf(final_message,
                    "%s: no change done for transaction with id=%li\n",
                    parameters.prog_name, parameters.id);
        } else {
        ...
        result = 1;
        sprintf(final_message,
                "%s: there is no transaction with id=%li - no change done\n",
                parameters.prog_name, parameters.id);
    }
...
int transaction_remove(PARAMETERS parameters)
{
    ...
    // Prepare SQL query and perform database actions
    sprintf(sql_remove_transaction,
            "DELETE FROM TRANSACTIONS WHERE TRANSACTION_ID=%li;",
            parameters.id);
    if (sqlite3_exec(db, sql_remove_transaction, NULL, NULL, &zErrMsg) !=
        SQLITE_OK) {
        fprintf(stderr, "%s: %s\n", parameters.prog_name, zErrMsg);
        sqlite3_free(zErrMsg);
        result = 1;
...
int transaction_list(PARAMETERS parameters)
{
    ...
    sprintf(sql_list_transactions, "SELECT" " T.TRANSACTION_ID" // 0
            ",T.YEAR"           // 1
            ",T.MONTH"          // 2
            ",T.DAY"            // 3
            ",T.DESCRIPTION"    // 4
    ...
    if (parameters.date[0] != NULL_STRING) {
        int year, month, day;
        DATE_TYPE date_type =
            date_from_string(parameters.date, &year, &month, &day);
        switch (date_type) {
        case DT_FULL_DATE:
            if (where_already_set == 0) {
                sprintf(sql_buf,
                        " WHERE T.YEAR=%d AND T.MONTH=%d AND T.DAY=%d",
                        year, month, day);
                where_already_set = 1;
            } else {
                sprintf(sql_buf,
                        " AND T.YEAR=%d AND T.MONTH=%d AND T.DAY=%d", year,
                        month, day);
            }
            break;
        case DT_MONTH:
            if (where_already_set == 0) {
                sprintf(sql_buf, " WHERE T.YEAR=%d AND T.MONTH=%d", year,
                        month);
                where_already_set = 1;
            } else {
                sprintf(sql_buf, " AND T.YEAR=%d AND T.MONTH=%d", year,
                        month);
            }
            break;
        case DT_YEAR:
    ...
    if (parameters.acc_name[0] != NULL_STRING) {
        if (where_already_set == 0) {
            sprintf(sql_buf, " WHERE A.NAME LIKE '%%%s%%'",
                    parameters.acc_name);
            where_already_set = 1;
        } else {
            sprintf(sql_buf, " AND A.NAME LIKE '%%%s%%'",
                    parameters.acc_name);
        }
        strncat(sql_list_transactions, sql_buf, BUF_SIZE);
    }
    if (parameters.cat_name[0] != NULL_STRING) {
        if (where_already_set == 0) {
            sprintf(sql_buf, " WHERE C.NAME LIKE '%%%s%%'",
                    parameters.cat_name);
            where_already_set = 1;
        } else {
            sprintf(sql_buf, " AND C.NAME LIKE '%%%s%%'",
                    parameters.cat_name);
        }
        strncat(sql_list_transactions, sql_buf, BUF_SIZE);
    }
    if (parameters.maincat_name[0] != NULL_STRING) {
        if (where_already_set == 0) {
            sprintf(sql_buf, " WHERE M.NAME LIKE '%%%s%%'",
                    parameters.maincat_name);
            where_already_set = 1;
        } else {
            sprintf(sql_buf, " AND M.NAME LIKE '%%%s%%'",
    ...
    if (parameters.maincategory_type != CAT_TYPE_NOTSET) {
        if (where_already_set == 0) {
            sprintf(sql_buf, " WHERE M.TYPE=%d",
                    parameters.maincategory_type);
            where_already_set = 1;
        } else {
            sprintf(sql_buf, " AND M.TYPE=%d",
                    parameters.maincategory_type);
        }
        strncat(sql_list_transactions, sql_buf, BUF_SIZE);
    }
    if (parameters.currency[0] != NULL_STRING) {
        if (where_already_set == 0) {
            sprintf(sql_buf, " WHERE A.CURRENCY='%s'",
                    parameters.currency);
            where_already_set = 1;
        } else {
    ...
    strncat(sql_list_transactions, " ORDER BY T.YEAR, T.MONTH, T.DAY;",
            BUF_SIZE);

    if (sqlite3_prepare_v2
        (db, sql_list_transactions, SQL_SIZE, &sqlStmt, NULL)
        != SQLITE_OK) {
        fprintf(stderr, "%s: %s\n", parameters.prog_name,
                sqlite3_errmsg(db));
        return 1;
    ...
    printf(FS_ID_T FS_GAP FS_DATE_T FS_GAP FS_NAME_T FS_GAP FS_MTYPE_T
           FS_GAP FS_NAME_T FS_GAP FS_NAME_T FS_GAP FS_VALUE_T FS_GAPS
           FS_CUR_T FS_GAP FS_DESC_T "\n", "ID", "DATE", "ACCOUNT", "TYPE",
           "MAIN CAT.", "CATEGORY", "VALUE", "CUR", "DESCRIPTION");
    ...
    while ((rc = sqlite3_step(sqlStmt)) == SQLITE_ROW) {
        ...
        printf(FS_GAP FS_MTYPE,
               maincategory_type_text(sqlite3_column_int(sqlStmt, 10)));
...
int budget_add(PARAMETERS parameters)
{
    ...
    if (parameters.dataFilePath[0] == NULL_STRING) {
        ...
        fprintf(stderr, MSG_MISSING_PAR_DATE, parameters.prog_name,
    ...
    // Prepare SQL query and perform database operations
    sprintf(sql_add_budget, "INSERT INTO BUDGETS VALUES (" "%d, "       // year
            "%d, "              // month
            "%li, "             // category id
            "round(%f,2), "     // value
            "'%s'"              // currency
            ");", year, month, category_id, budget_value,
            parameters.currency);

    if (sqlite3_exec(db, sql_add_budget, NULL, NULL, &zErrMsg) !=
        SQLITE_OK) {
        fprintf(stderr, "%s: %s\n", parameters.prog_name, zErrMsg);
        sqlite3_free(zErrMsg);
        result = 1;
...
int budget_list(PARAMETERS parameters)
{
    ...
    // Prepare SQL query and perform database actions
    int where_already_set = 0;
    sprintf(sql_list_budgets, "SELECT" " b.YEAR"        // 0
            ", b.MONTH"         // 1
            ", mc.TYPE"         // 2
            ", mc.NAME"         // 3
            ", c.NAME"          // 4
    ...
    if (parameters.date[0] != NULL_STRING) {
        int year, month;
        DATE_TYPE date_type =
            date_from_string(parameters.date, &year, &month, NULL);
        switch (date_type) {
        case DT_YEAR:
            if (where_already_set == 0) {
        ...
        case DT_MONTH:
            if (where_already_set == 0) {
                sprintf(sql_buf, " WHERE b.YEAR=%d AND b.MONTH=%d", year,
                        month);
                where_already_set = 1;
            } else {
                sprintf(sql_buf, " AND b.YEAR=%d AND b.MONTH=%d", year,
                        month);
            }
            break;
        default:
            fprintf(stderr, MSG_WRONG_DATE_MONTH_OR_YEAR,
                    parameters.prog_name);
            return 1;
        }
        strncat(sql_list_budgets, sql_buf, BUF_SIZE);
    }
    strncat(sql_list_budgets,
            " ORDER BY b.YEAR, b.MONTH, mc.TYPE, mc.NAME, c.NAME;",
            BUF_SIZE);

    if (sqlite3_prepare_v2(db, sql_list_budgets, SQL_SIZE, &sqlStmt, NULL)
        != SQLITE_OK) {
        fprintf(stderr, "%s: %s\n", parameters.prog_name,
                sqlite3_errmsg(db));
        return 1;
    ...
    printf(FS_MONTH_T FS_GAP FS_MTYPE_T FS_GAP FS_NAME_T FS_GAP FS_NAME_T
           FS_GAP FS_VALUE_T FS_GAPS FS_CUR_T "\n", "MONTH", "TYPE",
           "MAIN CAT.", "CATEGORY", "LIMIT", "CUR");

    // Print transactions on standard output
    while ((rc = sqlite3_step(sqlStmt)) == SQLITE_ROW) {
        printf(FS_MONTH, sqlite3_column_int(sqlStmt, 0), sqlite3_column_int(sqlStmt, 1));       // month
        printf(FS_GAP FS_MTYPE,
               maincategory_type_text(sqlite3_column_int(sqlStmt, 2)));
    ...
    if (budget_exists(db, category_id, year, month) == true) {
        ...
        if (parameters.value[0] != NULL_STRING) {
            budget_value =
                ...
                atof(parameters.value);
            sprintf(buf,
                    "UPDATE BUDGETS SET VALUE=round(%f,2)"
                    " WHERE YEAR=%d AND MONTH=%d AND CATEGORY_ID=%li;",
                    budget_value, year, month, category_id);
            strncat(sql_edit_budget, buf, BUF_SIZE);
        }
        if (parameters.currency[0] != NULL_STRING) {
            sprintf(buf, "UPDATE BUDGETS SET CURRENCY='%s'"
                    " WHERE YEAR=%d AND MONTH=%d AND CATEGORY_ID=%li;",
                    parameters.currency, year, month, category_id);
            strncat(sql_edit_budget, buf, BUF_SIZE);
        }

        strcat(sql_edit_budget, "COMMIT;");

        if (sqlite3_exec(db, sql_edit_budget, NULL, NULL, &zErrMsg) !=
            SQLITE_OK) {
            fprintf(stderr, "%s: %s\n", parameters.prog_name, zErrMsg);
            sqlite3_free(zErrMsg);
            result = 1;
            sprintf(final_message,
                    "%s: no change done for budget %d-%d %s\n",
                    parameters.prog_name, year, month,
                    parameters.cat_name);
        } else {
        ...
        result = 1;
        sprintf(final_message,
                "%s: there is no budget %d-%d %s - no change done\n",
...
int budget_remove(PARAMETERS parameters)
{
    ...
    sprintf(sql_remove_budget, "DELETE FROM BUDGETS WHERE"
            " YEAR=%d"
            " AND MONTH=%d"
            " AND CATEGORY_ID=%li" ";", year, month, category_id);
    if (sqlite3_exec(db, sql_remove_budget, NULL, NULL, &zErrMsg) !=
        SQLITE_OK) {
        fprintf(stderr, "%s: %s\n", parameters.prog_name, zErrMsg);
        sqlite3_free(zErrMsg);
        result = 1;
...
static bool account_exists(sqlite3 * db, const unsigned long account_id)
{
    ...
    sprintf(sql_account_exist,
            ...
            account_id, ITEM_STAT_OPEN);

    if (sqlite3_prepare_v2(db, sql_account_exist, SQL_SIZE, &sqlStmt, NULL)
        == SQLITE_OK) {
        if ((rc = sqlite3_step(sqlStmt)) == SQLITE_ROW) {
...
static bool
budget_exists(sqlite3 * db, const unsigned long category_id,
              const int year, const int month)
{
    ...
    if (sqlite3_prepare_v2(db, sql_budget_exists, SQL_SIZE, &sqlStmt, NULL)
        == SQLITE_OK) {
        if ((rc = sqlite3_step(sqlStmt)) == SQLITE_ROW) {
            if (sqlite3_column_int(sqlStmt, 0) != 0) {
                result = true;
...
static bool category_exists(sqlite3 * db, const unsigned long category_id)
{
    ...
    sprintf(sql_category_exists,
            ...
            category_id, ITEM_STAT_OPEN);

    if (sqlite3_prepare_v2
        (db, sql_category_exists, SQL_SIZE, &sqlStmt, NULL)
        == SQLITE_OK) {
        if ((rc = sqlite3_step(sqlStmt)) == SQLITE_ROW) {
            if (sqlite3_column_int(sqlStmt, 0) != 0) {
                result = true;
...
static bool
currency_exists(sqlite3 * db, const char *currency_from,
                const char *currency_to)
{
    ...
    if (sqlite3_prepare_v2
        (db, sql_currency_exists, SQL_SIZE, &sqlStmt, NULL)
        == SQLITE_OK) {
        if ((rc = sqlite3_step(sqlStmt)) == SQLITE_ROW) {
            if (sqlite3_column_int(sqlStmt, 0) != 0) {
                result = true;
...
static bool
maincategory_exists(sqlite3 * db, const unsigned long maincategory_id)
{
    ...
    sprintf(sql_maincategory_exists,
            ...
            maincategory_id, ITEM_STAT_OPEN);

    if (sqlite3_prepare_v2
        (db, sql_maincategory_exists, SQL_SIZE, &sqlStmt,
         NULL) == SQLITE_OK) {
        if ((rc = sqlite3_step(sqlStmt)) == SQLITE_ROW) {
            if (sqlite3_column_int(sqlStmt, 0) != 0) {
                result = true;
...
static bool
transaction_exists(sqlite3 * db, const unsigned long transaction_id)
{
    ...
    if (sqlite3_prepare_v2
        (db, sql_transaction_exists, SQL_SIZE, &sqlStmt,
         NULL) == SQLITE_OK) {
        if ((rc = sqlite3_step(sqlStmt)) == SQLITE_ROW) {
            if (sqlite3_column_int(sqlStmt, 0) != 0) {
                result = true;
